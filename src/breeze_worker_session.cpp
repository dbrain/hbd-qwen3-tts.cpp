    ...
    while (true) {
        ...
        switch (static_cast<WFrame>(hdr.type)) {
            ...
            case WFrame::SPEECH_REQ: {
                ...
                gen_result r;
                bool ok;
                // Long-form runs INSIDE the worker: it feeds each chunk's
                // generated codes back in as the next chunk's reference, and
                // SPEECH_RESP only carries PCM. Doing it server-side would mean
                // shipping codes back across the IPC every chunk.
                if (req.value("long", false)) {
                    ok = tts.synthesize_long(input, gp, ref.T ? &ref : nullptr,
